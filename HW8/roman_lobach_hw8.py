...
# Я тут зазделегіть подумав, що треба ж оформити return з декоруємої функції.
...
def timer_func(seconds, *args):
    '''
    Function imitate some 'hard process', using timer(time.sleep func). Main process.
    With some code before 'hard process', and some code after 'hard process'.
    Code before process returns min value of some positionated args
    Code after process returns max value of some positionated args

    Args:
        seconds (int | float): takes seconds for timer
        args (int | float): takes numbers to compare them for min and max value
    ...
    time.sleep(seconds)
    result_after_task = max(args)

    return result_before_task, result_after_task

print(timer_func(0.2, 10, 20, 30))
print(timer_func(0.4, 0, 300, 500))
print(timer_func(1, -23.75, 12.756, 53.525))
